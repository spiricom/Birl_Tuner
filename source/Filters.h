...
static inline DCFilter *initDCFilter(double gain) {
...
static inline double inputDCFilter(DCFilter *dc, double input) {
...
static inline Biquad *initBiquad() {
...
static inline void biquadSetCoeffs(Biquad *biquad, double a0, double a1, double a2, double b1, double b2) {
    biquad->a0 = a0;
    biquad->a1 = a1;
    biquad->a2 = a2;
    biquad->b1 = b1;
...
static inline double inputBiquad(Biquad *biquad, double input) {
...
static inline PoleZero *initPoleZero() {
...
static inline void setCoeffsPoleZero(PoleZero *pz, double b0, double b1, double a1) {
    pz->b0 = b0;
    pz->b1 = b1;
    pz->a1 = a1;
}

static inline void setA1PoleZero(PoleZero *pz, double a1) {
    pz->a1 = -a1;
}

static inline void setB0PoleZero(PoleZero *pz, double b0) {
    pz->b0 = b0;
}

static inline void setB1PoleZero(PoleZero *pz, double b1) {
    pz->b1 = b1;
}

static inline double inputPoleZero(PoleZero *pz, double input) {
    pz->x1 = input;
...
static inline double z1PoleZero(PoleZero *pz) {
    return pz->z1;
}
...
static inline void setQSVF(SVF *svf, double Q) {
    svf->k = 1.0/Q;
    svf->a1 = 1/(1 + svf->g*(svf->g + svf->k));
    svf->a2 = svf->g*svf->a1;
    svf->a3 = svf->g*svf->a2;
...
static inline double inputSVFPeak(SVF *svf, double input) {
...
static inline double inputSVFNotch(SVF *svf, double input) {
...
static inline double inputSVFLP(SVF *svf, double input) {
   double v1 = svf->a1*svf->ic1eq + svf->a2*(input - svf->ic2eq);
   double v2 = svf->ic2eq + svf->g*v1;
   svf->ic1eq = 2*v1 - svf->ic1eq;
   svf->ic2eq = 2*v2 - svf->ic2eq;
...
static inline double inputSVFBand(SVF *svf, double input) {
   double v1 = svf->a1*svf->ic1eq + svf->a2*(input - svf->ic2eq);
   double v2 = svf->ic2eq + svf->g*v1;
   svf->ic1eq = 2*v1 - svf->ic1eq;
   svf->ic2eq = 2*v2 - svf->ic2eq;
...
static inline double inputSVFHP(SVF *svf, double input) {
   double v1 = svf->a1*svf->ic1eq + svf->a2*(input - svf->ic2eq);
   double v2 = svf->ic2eq + svf->g*v1;
   svf->ic1eq = 2*v1 - svf->ic1eq;
   svf->ic2eq = 2*v2 - svf->ic2eq;
...
static inline double tanhClip(double input) {
    return tanh(input);
}

static inline double clip(double input, double min, double max) {
    if (input < min)
        input = min;
    else if (input > max)
        input = max;
...
// shaper
static inline double shaper(double input, double m_drive) {
...
static inline double reedTable(double input) {
